...
static void draw_trophy_unlocked(GuiState &gui, NpTrophyUnlockCallbackData &callback_data) {
    if (gui.trophy_window_frame_stage == 0 || gui.trophy_window_frame_stage == 2) {
        ImVec2 target_window_pos = ImVec2(0.0f, 0.0f);

        if (gui.trophy_window_frame_stage == 0)
        ...
        if (gui.trophy_window_frame_stage == 0 && gui.trophy_window_frame_count == 0) {
        ...
        } else if (gui.trophy_window_frame_stage == 0 && gui.trophy_window_pos.x > target_window_pos.x) {
            gui.trophy_window_pos.x -= TROPHY_MOVE_DELTA;
        } else if (gui.trophy_window_frame_stage == 2 && gui.trophy_window_pos.x < target_window_pos.x) {
            gui.trophy_window_pos.x += TROPHY_MOVE_DELTA;
        } else {
            gui.trophy_window_frame_stage++;
    ...
    if (gui.trophy_window_frame_stage == 1) {
        gui.trophy_window_frame_count++;

        if (gui.trophy_window_frame_count == TROPHY_WINDOW_STATIC_FRAME_COUNT) {
            gui.trophy_window_frame_stage = 2;
            gui.trophy_window_frame_count = 0;
        }
    }
}
...
void draw_trophies_unlocked(GuiState &gui) {
    ...
    if (!gui.trophy_unlock_display_requests.empty()) {
        if (gui.trophy_window_frame_stage == 3) {
            ...
            gui.trophy_window_frame_stage = 0;
            gui.trophy_window_frame_count = 0;
        } else {
            draw_trophy_unlocked(host, gui.trophy_unlock_display_requests.back());
        }
    }
}
