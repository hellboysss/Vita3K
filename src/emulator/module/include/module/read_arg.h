...
Arg read(CPUState &cpu, const std::tuple<ArgsLayout<Args...>, LayoutArgsStates<Args...>> &args, const MemState &mem) {
    ...
    if constexpr(std::is_same_v<Arg, module::vargs>) {
        ...
        if (!index) {
            layoutState.gpr_used = 0;
            layoutState.stack_used = 0;
            layoutState.float_used = 0;
        } else {
            layoutState = std::get<1>(args)[index - 1];
        }
